...
export class PersonalComponent implements OnInit {
  ...
  constructor( private spinner: NgxSpinnerService, private alertasService : AlertasService, private loginService: LoginService, private funcionGlobalServices : FuncionesglobalesService,
    private anomaliasService : AnomaliasService, private personalService :  PersonalService ) { 
  ...
  cerrarModal_registro(){
   ...
   mostrarInformacion(){ 
    ...
    this.spinner.show();
    this.personalService.get_mostrar_informacion(this.formParamsFiltro.value)
    .subscribe((res:any)=>{  

      console.log(res);

      this.spinner.hide();  
      this.personales= res.list_data;
    }, error => {
      this.spinner.hide();
      alert(JSON.stringify(error));
    },)
 ...
 async saveUpdate(){ 
  ...
  if ( this.flag_modoEdicion==false) { //// nuevo  
     ...
     const codigoAnom = await this.personalService.get_verificar_codigoAnomalia(this.formParams.value.codigo_Anomalia);
     if (codigoAnom) {
      Swal.close();
      this.alertasService.Swal_alert('error','El codigo de anomalia ya existe, verifique..');
      return;
     }   
    this.personalService.save_anomalia(this.formParams.value)
      .subscribe((res:any)=>{  
        Swal.close();    
     ...
     Swal.showLoading();    
    this.personalService.set_edit_anomalia(this.formParams.value , this.formParams.value.id_Anomalia)
      .subscribe((res:any)=>{  
        Swal.close();    
         this.mostrarInformacion();
 ...
 anular(objBD:any){
   ...
   .then((result)=>{
     if(result.value){
       ...
       Swal.fire({  icon: 'info', allowOutsideClick: false, allowEscapeKey: false, text: 'Espere por favor'  })
       Swal.showLoading();
       this.personalService.set_anular_anomalia(objBD.id_Anomalia).subscribe((res:any)=>{
         Swal.close();        
         if (res = 'ok') {            
           for (const item of this.personales) {
